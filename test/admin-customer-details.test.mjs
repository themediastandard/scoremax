...
test('customer directory is a compact read-only index with details navigation', () => {
  ...
  assert.match(customersTable, />Type<\/p>/)
  ...
  assert.match(customersTable, /grid grid-cols-12 items-start[^\"]*px-6 py-5/)
  assert.match(customersTable, /grid grid-cols-12 items-center[^\"]*border-t[^\"]*px-6 py-4/)
  assert.match(customersTable, /aria-labelledby=\{customerNameId\}/)
  assert.match(customersTable, /Total spent/)
