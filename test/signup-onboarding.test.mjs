...
test('zero-student parents recover inside booking while student accounts retain self selection', () => {
  ...
  assert.match(firstStudent, /Add a student to continue/)
  assert.match(firstStudent, /This parent account does not have an active student yet/)
  assert.match(firstStudent, /continue this booking as soon as they&apos;re saved/)
  assert.match(firstStudent, /Add Student & Continue/)
  assert.match(selection, /inactive[\s\S]*href="\/dashboard\/students"[\s\S]*My Students/)
  assert.match(book, /onStudentCreated=\{\(studentId\) => void loadStudents\(studentId, true\)\}/)
