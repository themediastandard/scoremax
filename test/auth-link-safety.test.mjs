...
const authCodeError = readSource('src/app/auth/auth-code-error/page.tsx')
const resendSignupConfirmation = readSource('src/app/auth/auth-code-error/ResendSignupConfirmationForm.tsx')
...
test('failed signup confirmations resend signup email instead of starting password recovery', () => {
  assert.match(callback, /url\.searchParams\.set\('type', type\)/)
  assert.match(callback, /type === 'signup'[\s\S]*next \?\? '\/book'/)
  assert.match(callback, /if \(type === 'signup'\) return '\/book'/)
  assert.match(callback, /return NextResponse\.redirect\(authErrorUrl\(type, next\)\)/)
  assert.match(authCodeError, /type === 'signup'/)
  assert.match(authCodeError, /<ResendSignupConfirmationForm \/>/)
  assert.match(resendSignupConfirmation, /supabase\.auth\.resend\(\{/)
  assert.match(resendSignupConfirmation, /type: 'signup'/)
  assert.match(resendSignupConfirmation, /emailRedirectTo: new URL\('\/book', APP_URL\)\.toString\(\)/)
  assert.match(resendSignupConfirmation, /captchaToken/)
  assert.match(resendSignupConfirmation, /Use the newest message/)
})
